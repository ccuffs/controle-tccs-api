...
/**
 * Função de teste para validar a lógica com datas simuladas (opcional)
 * Útil para testes e validação durante desenvolvimento
 *
 * @param {Date} dataSimulada - Data para simular o cálculo
 * @returns {Promise<{ano: number, semestre: number}>} Resultado do cálculo
 */
const testarAnoSemestreComDataSimulada = async (dataSimulada) => {
	try {
		// Buscar todos os períodos ordenados por ano e semestre
		const periodos = await model.AnoSemestre.findAll({
			order: [['ano', 'ASC'], ['semestre', 'ASC']]
