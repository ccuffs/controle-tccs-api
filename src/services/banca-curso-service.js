...
const retornaDocentesBancaPorCurso = async (req, res) => {
	try {
		...
		const docentesBanca = await bancaCursoRepository.obterDocentesBancaPorCurso(id);
...
const retornaCursosPorDocenteBanca = async (req, res) => {
	try {
		...
		const cursos = await bancaCursoRepository.obterCursosPorDocenteBanca(codigo);
...
const verificarDocenteBanca = async (req, res) => {
	try {
		...
		const podeParticipar = await bancaCursoRepository.verificarDocenteBanca(idCurso, codigoDocente);
